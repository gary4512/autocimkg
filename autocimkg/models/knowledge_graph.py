...
class KnowledgeGraph(BaseModel):
    ...
    def remove_duplicate_entities(self) -> None:
        """
        Removes duplicate entities (entities) by relying on the `__hash__` and `__eq__` methods of the `Entity` class.
    ...
    def remove_duplicate_relationships(self) -> None:
        """
        Removes duplicate relationships by relying on the `__hash__` and `__eq__` methods of the `Relationship` class.
